# Telegram 信号跟单机器人（Gate / WEEX / Bitget USDT 永续）
...
常用功能不用打字：输入框下方常驻一排按钮（📊 状态、📈 战绩、📜 最近交易、🧠 AI识别、⏸ 暂停开仓、▶️ 恢复开仓、⚙️ 实盘/模拟、🛑 全部平仓、📖 帮助），
...
| `/mode`（⚙️ 实盘/模拟） | 每个频道切换实盘/模拟：切到实盘会提醒每单打到止损亏多少、要再点一次确认；切回模拟马上生效（已开的实盘单照常管到平仓）。切换结果保存在服务器 `data/modes.json`，重启后照样有效；以后 config.yaml 又改了这个频道，就以 config.yaml 为准 |
| `/pause` / `/resume` | 暂停 / 恢复实盘开新仓（已有持仓照常管理） |
| `/closeall` | 紧急：平掉本程序开的全部实盘仓位、撤挂单，并暂停 |
| `/ip` | 服务器 IP（交易所 API 白名单填这个） |
| `/join 邀请链接` | 跟一个私人群/频道（会员群）。直接把 `t.me/+…` 链接发给机器人也行。链接只保存在服务器上，不会进 GitHub |
| `/gate KEY SECRET` | 设置 Gate API（先验证再保存，并删除你这条消息）；用 Bitget 时是 `/bitget KEY SECRET PASSPHRASE` |
| `/weex KEY SECRET PASSPHRASE` | 设置 WEEX API（同样先验证再保存、删除消息） |
| `/gatetest` / `/weextest` | 用最小数量的 BTC 实测下单、挂止损、止损触发平仓（开实盘或换交易所前做一次，约 0.01U）。测试的交易所不是现在用的那个也能测 |
...
- **仓位**：每单打到止损亏总权益的 10%（`risk_per_trade_pct`，含手续费估算；也可以用 `risk_per_trade_usdt` 设固定金额），跟信号写几倍杠杆无关。止损越近仓位越大，止损越远仓位越小。
...
- **止损**：交易所里一定有止损，程序挂了也有保护（Bitget 开仓单自带；Gate、WEEX 开仓后马上另挂一张「平掉整个仓位」的止损单，连试 3 次挂不上就立刻平仓，平仓后自动撤掉）。频道「提損 / 保本」只收紧不放宽，由程序盯盘执行（交易所原止损保留兜底）。
- **止盈**：按信号给的止盈个数分批（`tp_plans`）：3 个及以上 → tp1 减 30%、tp2 减 30%、tp3 清仓；2 个 → tp1 不减仓只把止损移到成本价、tp2 清仓；1 个 → 到了全平。由程序每 5 秒盯盘、到价用「只减仓」市价单平；第一止盈后止损移到开仓价。没给止盈的按 2R 设一个。
...
- **私人群 / 会员群**：config.yaml 里写 `- private: 名字`（仓库是公开的，邀请链接不写这里），再给机器人发 `/join 邀请链接`。监听账号还不在群里会自动加入。
  如果是**群组**（成员也能说话），只跟群主和管理员发的消息，普通成员说「BTC 市價多」不会下单；如果是频道，全部都跟。
