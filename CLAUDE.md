...
2. **默认只跑模拟盘。** 只有用户在当前对话里明确说开实盘，并且你提醒过「实盘会真实下单，每单打到止损约亏 X」（X 按 config.yaml 的 risk 设置说，2026-09-26 起是总权益的 10%）之后，才能改 `live_trading: true`，并把指定频道改成 `mode: live`。
